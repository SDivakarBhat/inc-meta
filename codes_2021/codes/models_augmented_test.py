	...
	def forward(self, batch, flag=None):
		...
		if not flag:
			...
			if self.args.phase =='base':
				...
				#base_train_embeddings = self.classifier(train_concat_features)
				#base_test_embeddings = self.classifier(test_concat_features)	
			...
			del(test_embeddings)
			#del(base_train_embeddings)
			#del(base_test_embeddings)
